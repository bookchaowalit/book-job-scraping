...
- The enabled collection surface is seventeen configured jobs, including
  owned-domain public-page SEO checks. Four news RSS jobs are `migrated` to
  `book-news-scraping`. Three jobs remain blocked: `flight_prices`,
  `money_opportunities`, and `ddproperty_condos` (parser ready; live Cloudflare
  JS challenge).
- Domain adapters also live in sibling repos (`book-news-scraping`,
  `book-property-scraping`, `book-restaurant-scraping`,
  `book-ecommerce-scraping`). News RSS is scheduled by `book-news-scraping`.

#!/usr/bin/env python3
"""Record a human review decision for a facebook.group-post.v1 capture.

This command is local and review-only.  It updates selected pending rows,
keeps ``outreach_status=not_contacted``, and appends a hash-only audit event.
It never opens Facebook, follows a permalink, writes a CRM record, or sends a
message.
"""
...
from scripts.check_facebook_group_capture import validate_group_capture_rows  # noqa: E402
from scripts.import_facebook_group_posts import GROUP_FIELDNAMES, MAX_INPUT_BYTES  # noqa: E402
...
def load_capture(path: Path) -> tuple[bytes, list[dict[str, str]]]:
    ...
    if reader.fieldnames != GROUP_FIELDNAMES:
        raise ValueError("headers_do_not_match_facebook_group_post_v1")
...
def apply_review(
    ...
    *,
    ...
    for number in selected:
        ...
        post_hash = hashlib.sha256(str(row.get("post_url") or "").encode("utf-8")).hexdigest()
        audit.append(
            {
                "reviewed_at": stamp,
                ...
                "post_url_sha256": post_hash,
                "input_sha256": str(input_sha256 or ""),
            }
        )

    pending_remaining = sum(
        str(row.get("review_decision") or "").strip() == "pending" for row in materialised
    )
    stats = {"selected": len(selected), "updated": len(selected), "pending_remaining": pending_remaining}
...
def write_review_outputs(
    ...
    _atomic_write_csv(output_path, rows, GROUP_FIELDNAMES)
    audit_path = output_path.parent / "facebook_group_posts_review_history.csv"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"facebook_group_review: ERROR: {exc}", file=sys.stderr)
        return 2
